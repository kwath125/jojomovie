...
<body>
		...
		<div class="searchTop">
			<div class="frm1">
				<form action="/movie/moviePresent" method="get">
					<button class="categoryBtn" type="submit" name="category" value="score">전문가평점순</button>
					...
					<c:forEach var="aboutmovie" items="${movieList }">
						<div class="col-sm-4">
							<div class="thumbnail thumbnailBox">
								<img src="/images/${ aboutmovie.movieName }.jpg">
								<div class="caption">
									...
									<div class="thumbnailBtns">
										...
										<a href="#" class="thumbnailBtn">예매하기</a>
									</div>	
								</div>
							</div>
						</div>
	...
	<script>
		/* $('#connection').click(function(){
		 ...
		 obj.movieId = $('.movId').html();
		 obj.score = $('.sco').html();
		 obj.information = $('.infor').val();
		 arr.push(obj);
		 ...
		 $.ajax({
		 url: '/comment/list?movieId=' + movieAddress,
		 type: "post",
		...
		/* $('#connection').click(function(){
		 var movieId = $('.movId').html();
		 var score = $('.sco').html();
		 var inform = $('.infor').val();
		 console.log(movieId);
		 alert(movieId);
		 console.log(scor);
		 alert(scor);
		...
		function list(tab){
				...
				$.ajax({
						...
						var content = html.find("div#indexListAjax").html();
						if(tab == "ing"){
									$("#tabl1").html(contents);
							} else if(tab == "end"){
									$("tabl2").html(contents);
